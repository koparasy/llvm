...
static bool RenderModulesOptions(Compilation &C, const Driver &D,
  ...
  if (Args.hasFlagNoClaim(options::OPT_fmodules, options::OPT_fno_modules,
                          false)) {
...
void OffloadBundler::ConstructJobMultipleOutputs(
  ...
  // Get bundled file command.
  CmdArgs.push_back(TCArgs.MakeArgString(Twine("-input=") + InputFileName));
...
static void addRunTimeWrapperOpts(Compilation &C,
                                  ...
                                  const llvm::opt::ArgList &TCArgs,
                                  ArgStringList &CmdArgs, const ToolChain &TC,
                                  const JobAction &JA) {
  ...
  if (TT.getSubArch() == llvm::Triple::NoSubArch) {
    ...
    const ArgList &Args =
        C.getArgsForToolChain(nullptr, StringRef(), DeviceOffloadKind);
...
void SPIRVTranslator::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  for (auto I : Inputs) {
    ...
    if (I.getType() == types::TY_Tempfilelist) {
      ...
      ForeachArgs.push_back(C.getArgs().MakeArgString("--out-ext=spv"));
    }
  ...
  if (!ForeachArgs.empty()) {
    ...
    C.addCommand(std::make_unique<Command>(JA, *this,
                                           ResponseFileSupport::None(), Foreach,
                                           ForeachArgs, ArrayRef<InputInfo>{}));
  } else
    C.addCommand(std::move(Cmd));
}
