...
static_assert(std::is_same_v<decltype(sycl::popcount(MArray2i{1, 3})),
                             MArray2i>);
static_assert(std::is_same_v<decltype(sycl::mad24(std::int32_t{2},
                                                  std::int32_t{3},
                                                  std::int32_t{4})),
                             std::int32_t>);
static_assert(std::is_same_v<decltype(sycl::mul24(Vec2i{2, 3}, Vec2i{4, 5})),
                             Vec2i>);
static_assert(std::is_same_v<decltype(sycl::mul24(MArray2i{2, 3},
                                                  MArray2i{4, 5})),
                             MArray2i>);
static_assert(
    std::is_same_v<decltype(sycl::upsample(Vec2s{1, 2}, Vec2us{3, 4})), Vec2i>);
static_assert(std::is_same_v<decltype(sycl::upsample(MArray2s{1, 2},
                                                     MArray2us{3, 4})),
                             MArray2i>);
