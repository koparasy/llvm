...
static_assert(std::is_same_v<decltype(sycl::cross(MArray3f{1.0f, 0.0f, 0.0f},
                             ...
                             MArray3f>);
static_assert(std::is_same_v<decltype(sycl::normalize(Vec3f{1.0f, 2.0f, 3.0f})),
                             Vec3f>);
static_assert(std::is_same_v<decltype(sycl::normalize(MArray3f{1.0f, 2.0f, 3.0f})),
                             MArray3f>);

int main() {
  // Instantiate representative geometric calls that should compile via the wrapper.
