...
Applications that need the object-returning APIs from this extension should
include:
...
This header provides the `this_work_item::get_nd_item`,
`this_work_item::get_work_group`, and `this_work_item::get_sub_group` APIs, as
well as the deprecated compatibility wrappers described below.

Applications that only need scalar work-item queries may instead include:
...
This header provides a lighter-weight entry point into the same
`sycl::ext::oneapi::this_work_item` namespace for scalar queries such as global,
local, and group ids / ranges and linear ids. It is intended for code paths
where compile time matters and the object-returning APIs from this extension are
not required.
