...
struct LaunchSizeListToStrHelper<LaunchSizeList<Value, Values...>,
  ...
  : LaunchSizeListToStrHelper<LaunchSizeList<Value / LaunchDecimalBase,
                         Values...>,
                                LaunchCharList<ParsedChars...>,
                '0' + (Value % LaunchDecimalBase), Chars...> {
};
...
template <size_t Dim0, size_t... Dims> struct work_group_size_t;
template <size_t Dim0, size_t... Dims> struct work_group_size_hint_t;
template <uint32_t Size> struct sub_group_size_t;
template <size_t Dim0, size_t... Dims> struct max_work_group_size_t;
template <size_t Size> struct max_linear_work_group_size_t;

} // namespace detail

struct work_group_size_key
    : detail::compile_time_property_key<detail::PropKind::WorkGroupSize> {
  template <size_t... Dims> using value_t = detail::work_group_size_t<Dims...>;
};

struct work_group_size_hint_key
    : detail::compile_time_property_key<detail::PropKind::WorkGroupSizeHint> {
  template <size_t... Dims>
  using value_t = detail::work_group_size_hint_t<Dims...>;
};

struct sub_group_size_key
    : detail::compile_time_property_key<detail::PropKind::SubGroupSize> {
  template <uint32_t Size> using value_t = detail::sub_group_size_t<Size>;
};

struct max_work_group_size_key
    : detail::compile_time_property_key<detail::PropKind::MaxWorkGroupSize> {
  template <size_t... Dims>
  using value_t = detail::max_work_group_size_t<Dims...>;
};
...
struct max_linear_work_group_size_key
  ...
  using value_t = detail::max_linear_work_group_size_t<Size>;
};

namespace detail {

template <size_t Dim0, size_t... Dims>
struct work_group_size_t
    : property_base<work_group_size_t<Dim0, Dims...>, PropKind::WorkGroupSize,
                    work_group_size_key> {
  ...
  static_assert(LaunchAllNonZero<Dim0, Dims...>::value,
  ...
  static constexpr size_t size() { return sizeof...(Dims) + 1; }
};

template <size_t Dim0, size_t... Dims>
struct work_group_size_hint_t
    : property_base<work_group_size_hint_t<Dim0, Dims...>,
                    PropKind::WorkGroupSizeHint, work_group_size_hint_key> {
  static constexpr PropKind Kind = PropKind::WorkGroupSizeHint;

    static_assert(
      sizeof...(Dims) + 1 <= 3,
      "work_group_size_hint property currently only supports up to three values.");
  static_assert(LaunchAllNonZero<Dim0, Dims...>::value,
...
struct sub_group_size_t
    : property_base<sub_group_size_t<Size>, PropKind::SubGroupSize,
                    sub_group_size_key> {
...
template <size_t Dim0, size_t... Dims>
struct max_work_group_size_t
    : property_base<max_work_group_size_t<Dim0, Dims...>,
                    PropKind::MaxWorkGroupSize, max_work_group_size_key> {
  static constexpr PropKind Kind = PropKind::MaxWorkGroupSize;
...
template <size_t Size>
struct max_linear_work_group_size_t
    : property_base<max_linear_work_group_size_t<Size>,
                    PropKind::MaxLinearWorkGroupSize,
                    max_linear_work_group_size_key> {
  static constexpr PropKind Kind = PropKind::MaxLinearWorkGroupSize;
